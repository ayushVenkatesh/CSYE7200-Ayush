...
object WebCrawler extends App {
    ...
    def wget(url: URL)(implicit ec: ExecutionContext): Future[Seq[URL]] = {
        ...
        // 16 points.
        def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a") yield createURL(Option(url), n.toString) // TO BE IMPLEMENTED
        ...
        // 9 points.
        for {c <- getURLContent(url); x <- MonadOps.asFuture(getLinks(c))} yield x // TO BE IMPLEMENTED
    }
