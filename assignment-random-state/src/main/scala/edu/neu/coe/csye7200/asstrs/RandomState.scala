...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  // 7 points
  val myRandom: Random = new Random(n)
  def next: RandomState[T] = JavaRandomState(myRandom.nextLong(), g) // TO BE IMPLEMENTED
  // Hint: Think of the input and output.
  // 5 points
  def get: T = g(n) // TO BE IMPLEMENTED
  // Hint: This one need function composition.
  // 13 points
  def map[U](f: T => U): RandomState[U] = flatMap(t => JavaRandomState(n,g andThen(f))) // TO BE IMPLEMENTED
}
